...
function handleSubmit(payload) {
  ...
  if (row > 0) {
    ...
    // Only update submission count if the column exists (skip for old 6-col sheets)
    if (cm.count >= 0) {
      var currentCount = sheet.getRange(row, cm.count + 1).getValue() || 0;
      sheet.getRange(row, cm.count + 1).setValue(currentCount + 1);
    }
    // Write data to the correct column (use column map, no hardcoded fallback)
    if (cm.data >= 0) sheet.getRange(row, cm.data + 1).setValue(dataJson);
    sheet.getRange(row, subjectCol).setValue(subject);
  } else {
    // Build row array dynamically based on actual column layout
    var maxCol = Math.max(cm.timestamp, cm.name, cm.class, cm.wsId, cm.data, cm.subject);
    if (cm.count >= 0) maxCol = Math.max(maxCol, cm.count);
    if (cm.wsTitle >= 0 && cm.wsTitle !== cm.wsId) maxCol = Math.max(maxCol, cm.wsTitle);
    var newRow = [];
    for (var c = 0; c <= maxCol; c++) newRow.push('');

    newRow[cm.timestamp] = now;
    newRow[cm.name >= 0 ? cm.name : 1] = name;
    newRow[cm.class >= 0 ? cm.class : 2] = cls;
    newRow[cm.wsId >= 0 ? cm.wsId : 3] = wsId;
    if (cm.wsTitle >= 0 && cm.wsTitle !== cm.wsId) newRow[cm.wsTitle] = wsTitle;
    if (cm.count >= 0) newRow[cm.count] = 1;
    newRow[cm.data >= 0 ? cm.data : 4] = dataJson;
    if (cm.subject >= 0) newRow[cm.subject] = subject;
...
function handleExport(params) {
  ...
  for (var i = 1; i < data.length; i++) {
    var row = data[i];
    var rowClass = colVal(row, cmExp.class);
    var rowSubject = (colVal(row, cmExp.subject) || 'technology').toString().toLowerCase();
    ...
    var appData = {};
    try { appData = JSON.parse(colVal(row, cmExp.data) || '{}'); } catch (e) {}
    var s = buildStudentSummary(appData);

    var countVal = cmExp.count >= 0 ? colVal(row, cmExp.count) : 1;
    var fbVal = cmExp.feedback >= 0 ? colVal(row, cmExp.feedback) : '';

    csvRows.push([
      csvEscape(colVal(row, cmExp.name)),
      csvEscape(rowClass),
      csvEscape(s.band),
      s.modulesCompleted,
      s.totalModules,
      ...
      s.evidenceScore,
      countVal,
      csvEscape(colVal(row, cmExp.timestamp)),
      fbVal ? 'Yes' : 'No'
    ].join(','));
  }
...
function handleGiveFeedback(payload) {
  ...
  // Find or create feedback columns (using buildColMap for consistency)
  var headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
  var cm = buildColMap(headers);
  var fbCol = cm.feedback;  // 0-based index, or -1
  var fdCol = cm.feedbackDate;  // 0-based index, or -1
  if (fbCol === -1) {
    fbCol = headers.length;  // 0-based = next available column
...
function handleBatchFeedback(payload) {
  ...
  // Find feedback columns dynamically
  var headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
  var cm = buildColMap(headers);
  var fbCol = cm.feedback >= 0 ? cm.feedback + 1 : -1;
  var fdCol = cm.feedbackDate >= 0 ? cm.feedbackDate + 1 : -1;

  // Create feedback columns if they don't exist yet
  if (fbCol < 0) {
    fbCol = headers.length + 1;
    sheet.getRange(1, fbCol).setValue('FeedbackText').setFontWeight('bold');
    fdCol = fbCol + 1;
    sheet.getRange(1, fdCol).setValue('FeedbackDate').setFontWeight('bold');
  }
  if (fdCol < 0) {
    fdCol = fbCol + 1;
    sheet.getRange(1, fdCol).setValue('FeedbackDate').setFontWeight('bold');
  ...
  for (var i = 0; i < targets.length; i++) {
    ...
    var row = findStudentRow(sheet, name, cls, wsId);
    if (row > 0) {
      sheet.getRange(row, fbCol).setValue(feedback);
      sheet.getRange(row, fdCol).setValue(now);
      updated++;
    } else {
      notFound.push(name);
    }
